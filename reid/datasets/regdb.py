...
class Regdb(Dataset):
    ...
    def __init__(self, root, split_id=0, num_val=30, download=True):
    ...
    def download(self):
        ...
        fpath = osp.join(raw_dir, 'regdb_v2.zip')
        ...
        # 412 identities (+1 for background) with 3 camera views each
        identities = [[[] for _ in range(3)] for _ in range(413)]
        # print("identities=",identities)

        def register(subdir, pattern=re.compile(r'([-\d]+)_c(\d)')):
            fnames = []
            fpaths = sorted(glob(osp.join(exdir, subdir, '*.bmp'))) #
            pids = set()
            for fpath in fpaths:
